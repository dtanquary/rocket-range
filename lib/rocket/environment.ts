...
export function buildEnvironment(scene: T.Scene) {
  ...
  const sun = new T.DirectionalLight("#fff0ca", 3.8);
  ...
  for (let i = 0; i < a.count; i++) {
    ...
    a.setY(
      ...
      d < 900
        ...
        : Math.max(
            0,
            Math.sin(x * 0.004 + 1) * Math.cos(z * 0.005) * 38 +
              Math.sin(z * 0.011 + x * 0.003) * 12,
          ) *
            Math.min(1, (d - 900) / 200) -
            0.025,
    );
  }
  groundGeo.computeVertexNormals();
  mesh(groundGeo, groundMat, root).castShadow = false;
  ...
  blade.setAttribute(
    ...
    new T.Float32BufferAttribute(
      [
        -0.018, 0, 0, 0.018, 0, 0, -0.011, 0.075, 0.006, 0.011, 0.075, 0.006, 0,
        0.15, 0.025,
      ],
      3,
    ),
  ...
  const grass = new T.InstancedMesh(blade, grassMat, 38000);
  const dummy = new T.Object3D();
  const col = new T.Color();
  for (let i = 0; i < 38000; i++) {
    const angle = rand() * Math.PI * 2,
      d = Math.sqrt(rand()) * (i < 22000 ? 22 : 105);
    const x = Math.cos(angle) * d,
      z = Math.sin(angle) * d;
    dummy.position.set(x, -0.015, z);
    dummy.rotation.y = rand() * Math.PI;
    const scale = d < 1.1 ? 0.03 : 0.45 + rand() * 0.7;
    dummy.scale.set(scale, scale * (d < 12 ? 0.5 : 1), scale);
    dummy.updateMatrix();
  ...
  const trunks = new T.InstancedMesh(
    new T.CylinderGeometry(0.16, 0.26, 1, 7),
    new T.MeshStandardMaterial({ color: "#514638", roughness: 1 }),
    230,
  );
  const leaves = new T.InstancedMesh(
    new T.IcosahedronGeometry(1, 2),
    new T.MeshStandardMaterial({ color: "#354c2c", roughness: 1 }),
    230 * 5,
  );
  for (let i = 0; i < 230; i++) {
    const angle = rand() * Math.PI * 2,
  ...
  for (let i = -16; i <= 16; i++) {
    const x = i * 5;
    mesh(new T.BoxGeometry(0.15, 1.2, 0.15), wood, root, x, 0.6, -82);
    if (i < 16)
      for (const y of [0.45, 0.94])
        mesh(new T.BoxGeometry(5, 0.095, 0.095), wood, root, x + 2.5, y, -82);
  }
  // Preparation table, with real launch controller and a field equipment case.
  const bench = new T.Group();
  bench.position.set(1.4, 0, 0.35);
  root.add(bench);
  mesh(new T.BoxGeometry(0.85, 0.04, 0.5), wood, bench, 0, 0.69, 0);
  for (const x of [-0.34, 0.34])
    for (const z of [-0.17, 0.17])
      beam(
        bench,
        new T.Vector3(x, 0, z),
        new T.Vector3(x * 0.8, 0.67, z),
        0.018,
        materials.steel,
      );
  const controller = createController();
  controller.position.set(0.22, 0.711, 0);
  controller.rotation.y = -0.25;
  bench.add(controller);
  mesh(new T.BoxGeometry(0.34, 0.2, 0.25), materials.black, root, 2, 0.1, 0.5);
  // A striped windsock and boundary cones.
  const sock = new T.Group();
  sock.position.set(-5, 0, -5);
  root.add(sock);
  cylinder(sock, 0.022, 0.022, 3.1, 1.55, materials.steel);
  const fabric = new T.Group();
  fabric.position.y = 3;
  fabric.rotation.z = -Math.PI / 2;
  fabric.rotation.x = 0.16;
  sock.add(fabric);
  for (let i = 0; i < 6; i++) {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    cone.position.set(x, 0, 8);
    root.add(cone);
    mesh(
      new T.BoxGeometry(0.22, 0.022, 0.22),
      materials.black,
      cone,
      0,
      0.011,
    ...
    cylinder(
      ...
      0.28,
      0.16,
      new T.MeshStandardMaterial({ color: "#e56b2c" }),
    );
    cylinder(
      cone,
      0.04,
      ...
      0.04,
      0.19,
      new T.MeshStandardMaterial({ color: "#f0eadd" }),
    );
  }
  // A controller lead resting on the grass.
  const points = [
    new T.Vector3(1.6, 0.73, 0.35),
    new T.Vector3(1.85, 0.2, 0.5),
    new T.Vector3(1.4, 0.013, 0.8),
    new T.Vector3(0.6, 0.013, 0.65),
    new T.Vector3(0.25, 0.02, 0.3),
    new T.Vector3(0, 0.14, 0),
  ];
  mesh(
    new T.TubeGeometry(new T.CatmullRomCurve3(points), 60, 0.0025, 5, false),
    materials.black,
    root,
  );
