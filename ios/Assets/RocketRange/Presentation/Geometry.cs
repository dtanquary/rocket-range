...
namespace RocketRange
{
    public static class Geometry
    {
        ...
        public static Material Paint(string hex, float metallic = 0, float smoothness = .3f)
        {
            ...
            m.SetColor("_BaseColor", Color(hex));
        ...
        )
        {
            var mesh = Lathe(
                ...
                {
                    new Vector2(0, 0),
                    new Vector2(bottom, 0),
                    new Vector2(top, height),
                    new Vector2(0, height)
                },
                sides
            );
            var g = Mesh(name, mesh, mat, parent);
        ...
        public static UnityEngine.Mesh Lathe(Vector2[] profile, int sides = 40)
        {
            ...
            for (int j = 0; j < profile.Length; j++)
                for (int i = 0; i <= sides; i++)
                {
                    ...
                    vertices.Add(
                        new Vector3(
                            Mathf.Cos(a) * profile[j].x,
                            profile[j].y,
                            Mathf.Sin(a) * profile[j].x
                        )
                    );
                    ...
                    if (j > 0 && i > 0)
                    {
                        int k = j * (sides + 1) + i;
                        triangles.AddRange(
                            new[] { k, k - sides - 2, k - 1, k, k - sides - 1, k - sides - 2 }
                        );
                    }
                }
            var m = new UnityEngine.Mesh { name = "Lathe" };
        ...
        )
        {
            ...
            g.transform.localPosition = start;
            g.transform.localRotation = Quaternion.FromToRotation(
                Vector3.up,
        ...
        public static UnityEngine.Mesh Fin(float radius, float span, float height)
        {
            ...
            var v = new[]
            {
                new Vector3(radius, 0, -thick),
                new Vector3(radius + span, 0, -thick),
                new Vector3(radius + span, height * .43f, -thick),
                new Vector3(radius, height, -thick),
                new Vector3(radius, 0, thick),
                new Vector3(radius + span, 0, thick),
                new Vector3(radius + span, height * .43f, thick),
                new Vector3(radius, height, thick)
