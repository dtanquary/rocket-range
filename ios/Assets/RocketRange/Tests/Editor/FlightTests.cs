...
namespace RocketRange.Tests
{
    public class FlightTests
    {
        ...
        [Test]
        public void AllEightyTwoFlightsMatchBrowserReference()
        {
            var data = JsonUtility.FromJson<ReferenceSet>(
                Resources.Load<TextAsset>("Data/flight-reference").text
            );
            Assert.That(data.fixtures.Length, Is.EqualTo(82));
            foreach (var f in data.fixtures)
            {
                var s = FlightPhysics.Predict(
                    new FlightConfiguration(
                        catalog.Rocket(f.rocket),
                        catalog.Motor(f.motor),
                        f.conditions
                    )
                );
                ...
                foreach (
                    var p in new[]
                    {
                        ...
                        (s.impactSpeed, f.impactSpeed),
                        (s.deployedAt ?? -1, f.deployedAt),
                        (s.rodExitTime ?? -1, f.rodExitTime)
                    }
                )
                    Assert.That(p.Item1, Is.EqualTo(p.Item2).Within(.00002), name);
                Assert.That(
                    s.outcome.ToString().ToLowerInvariant(),
                    Is.EqualTo(f.outcome.Replace("-", "")),
                    name
                );
            }
        }
        ...
        [Test]
        public void CompatibilityChecksMountAndVariant()
        {
            var r = catalog.rockets[0];
            var m = catalog.Motor("B6-4");
            Assert.IsTrue(FlightConfiguration.Fits(r, m));
            Assert.Throws<ArgumentException>(
                () => new FlightConfiguration(r, catalog.Motor("D12-3"), Conditions.Default)
            );
            m.length = 95;
            Assert.IsFalse(FlightConfiguration.Fits(r, m));
            m.length = 70;
            m.diameter = 24;
            Assert.IsFalse(FlightConfiguration.Fits(r, m));
        }
        ...
        [Test]
        public void SnapshotDoesNotShareMutableInputs()
        {
            var r = catalog.rockets[0];
            var m = catalog.Motor("B6-4");
            var c = Conditions.Default;
            var f = new FlightConfiguration(r, m, c);
            var expected = FlightPhysics.Predict(f);
            r.mass = 999;
            r.motors[0] = "invalid";
            m.curve[1].force = 999;
            c.wind = 6;
            var exposed = f.Rocket;
            exposed.mass = 888;
            f.Motor.curve[1].force = 888;
        ...
        public void IgniterDelayLocksSetupAndStartsClockAtIgnition(double random, double delay)
        {
            ...
            s.Advance(delay - .01);
            Assert.That(s.State.t, Is.Zero);
            s.Advance(.01 + FlightPhysics.Step);
            Assert.That(s.State.t, Is.EqualTo(FlightPhysics.Step).Within(1e-9));
        }

        [Test]
        public void FramePacingAndBackgroundCatchUpDoNotChangeFlight()
        {
            var a = Ready();
            var b = Ready();
            a.Launch(.5);
            b.Launch(.5);
            for (int i = 0; i < 30000 && a.Stage != Preparation.Landed; i++)
                a.Advance(1.0 / 60);
            b.Advance(300);
            ...
            a.PrepareAgain();
            Assert.That(a.State.t, Is.Zero);
            Assert.That(a.FlightNumber, Is.EqualTo(2));
        }

        [Test]
        public void EjectionOccursAfterBurnoutAndCanopySlowsDescent()
        {
            var r = catalog.rockets[0];
            var m = catalog.Motor("B6-4");
            ...
            Assert.That(
                s.deployedAt.Value,
                Is.EqualTo(m.burn + m.delay).Within(FlightPhysics.Step)
            );
        ...
        [Test]
        public void RecoveryFailureIsPhysicalRatherThanForcedSuccess()
        {
            var r = catalog.rockets[0];
            var m = catalog.Motor("C6-5");
            m.delay = 0;
            var early = FlightPhysics.Predict(new FlightConfiguration(r, m, Conditions.Default));
            Assert.IsTrue(early.chuteFailed);
            Assert.That(early.outcome, Is.EqualTo(FlightOutcome.HardLanding));
            m.delay = 25;
