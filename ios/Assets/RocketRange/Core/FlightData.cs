...
namespace RocketRange.Core
{
    ...
    public sealed class Rocket
    {
        ...
        public Rocket Copy()
        {
            var r = (Rocket)MemberwiseClone();
            r.motors = (string[])motors.Clone();
            return r;
        }
    }
    ...
    public sealed class Motor
    {
        ...
        public Motor Copy()
        {
            var m = (Motor)MemberwiseClone();
            m.curve = (ThrustSample[])curve.Clone();
            return m;
        }
    }
        ...
        public void Validate()
        {
            foreach (
                ...
                if (double.IsNaN(value) || double.IsInfinity(value))
                    throw new ArgumentException("Conditions must be finite.");
            if (
                wind < 0
                || wind > 6
                || angle < 0
    ...
    public sealed class FlightConfiguration
    {
        ...
        {
            if (!Fits(r, m))
                throw new ArgumentException("Engine is incompatible with this airframe.");
            c.Validate();
            rocket = r.Copy();
            motor = m.Copy();
            conditions = c;
        ...
        public static bool Fits(Rocket r, Motor m) =>
            r.motors.Contains(m.id) && m.diameter == r.mountDiameter && m.length <= r.mountLength;
    }
    ...
    public struct FlightState
    {
        public double t,
            ...
            impactSpeed,
            maxAngleOfAttack;
        public double? deployedAt,
            rodExitTime,
            rodExitSpeed,
            deploymentSpeed;
        public DVec position,
            velocity,
            axis,
            omega;
        ...
        public static FlightState New(Conditions c) =>
            new FlightState { axis = FlightPhysics.LaunchAxis(c) };
    }
